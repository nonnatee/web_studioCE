...
class StudioEditor(models.AbstractModel):
    ...
    def get_view_arch_and_fields(self, model_name, view_id, view_type):
        ...
        # Retrieve custom extension view details
    ...
    @api.model
    def set_custom_view_arch(self, view_id, arch):
        """
    ...
    def edit_view_attribute(self, view_id, target_xpath, attribute_name, attribute_value):
        ...
        # Find matching xpath element safely without string formatting interpolation issues
        xpath_node = None
        for child in arch_xml:
            if child.tag == 'xpath' and child.get('expr') == target_xpath and child.get('position') == 'attributes':
                xpath_node = child
                break
        ...
        for child in xpath_node:
            if child.tag == 'attribute' and child.get('name') == attribute_name:
                attr_node = child
                break
    ...
    def add_field_to_view(self, model_name, view_id, target_xpath, position, field_name, field_label=None, field_type=None, relation_model=None, selection_values=None):
        ...
        # Safe matching for existing xpath element
        xpath_node = None
        for child in arch_xml:
            if child.tag == 'xpath' and child.get('expr') == target_xpath and child.get('position') == position:
                xpath_node = child
                break
